...
const updateRecordInSession = (sessions, sessionId, recordId, updater) => sessions.map((session) => session.id === sessionId
    ? {
        ...session,
...
export const useAppStore = create((set, get) => ({
    ...
    getTranslationModelByLanguage: (lang) => get().models.find((model) => model.type === "translation" && model.language === lang),
    isVoiceModelInstalled: () => get().models.some((model) => model.type === "asr" && model.status === "installed"),
    ...
    downloadModel: (modelId, notify) => {
        ...
        if (!model)
            return;
        modelService.downloadModel(modelId, model.progress ?? 0, (patch) => {
            set((state) => ({
                models: state.models.map((item) => (item.id === modelId ? { ...item, ...patch } : item))
            }));
        }, () => notify(`${model.language ?? "语音"}模型下载完成`, "success"), () => notify(`${model.language ?? "语音"}模型下载失败`, "error"));
    },
    ...
    cancelDownload: (modelId, notify) => {
        ...
        set((state) => ({
            models: state.models.map((item) => item.id === modelId ? { ...item, status: "not_installed", progress: 0 } : item)
        }));
        notify("下载已取消", "info");
    },
...
export const findLanguageLabel = (codeOrLabel) => languageOptions.find((item) => item.code === codeOrLabel || item.label === codeOrLabel)?.label ?? codeOrLabel;
